...
public class VanishCommand extends Command {
    ...
    public void execute(final Player player, final String[] args) {
        ...
        if (args == null || args.length == 0) {
            ...
            getInstance().getClientUtilities().messageStaff("Vanish", ChatColor.YELLOW + player.getName() + ChatColor.GRAY + (client.isVanished() ? " is now Invisible." : " is no longer Invisible."), Rank.ADMIN, null);
            return;
        }
        if (args.length == 1) {
            ...
            getInstance().getClientUtilities().messageStaff("Vanish", ChatColor.YELLOW + target.getName() + ChatColor.GRAY + (targetC.isVanished() ? " is now Invisible " : " is no longer Invisible ") + " by " + ChatColor.YELLOW + player.getName() + ChatColor.GRAY + ".", Rank.ADMIN, null);
        }
    }
